<?php
/**
 * emergencia.php â€” MÃ³dulo de Emergencias
 * Hochschild Mining â€” Sistema Integral de Transporte
 */
...
// â”€â”€â”€ AJAX HANDLERS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] === 'pasajeros_bus') {
        ...
        // Si la placa fue ingresada manualmente y no hay coincidencias, devolver vacÃ­o
...
<title>MÃ³dulo de Emergencias Â· Hochschild Mining</title>
...
<header class="topbar">
    <div class="topbar-left">
        ...
        <div>
            <div class="module-title">ðŸš¨ EMERGENCIAS</div>
...
<div class="print-header">
    <h1>REPORTE DE EMERGENCIA â€” HOCHSCHILD MINING</h1>
    <p id="printMeta"></p>
</div>
...
<div class="main">
    ...
    <div class="panel active" id="panelNueva">
        ...
        <div class="trigger-zone" id="triggerZone">
            <div class="trigger-label">â€” Activar protocolo de emergencia â€”</div>
            <button class="sos-btn" id="sosBtn" onclick="armSOS()">
                <span class="sos-text">SOS</span>
                <span class="sos-sub">ACTIVAR</span>
            </button>
            <p class="arm-hint" id="armHint">Presiona para armar Â· Segunda pulsaciÃ³n confirma</p>
        </div>
        ...
        <div id="formEmergencia" style="display:none">
                ...
                <div class="form-row" style="margin-bottom:12px">
                    <div class="form-group">
                        ...
                        <select id="tipoIncidente">
                            ...
                            <option>Falla mecÃ¡nica grave</option>
                            <option>Derrumbe / Huayco</option>
                            <option>Asalto / Robo</option>
                            <option>Emergencia mÃ©dica a bordo</option>
                            <option>Otro</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    <label>UbicaciÃ³n del incidente</label>
            ...
            <!-- BUS -->
            <div class="form-section">
                <div class="form-section-title"><i class="fas fa-bus"></i> IdentificaciÃ³n del Bus</div>
            ...
            <div class="form-section">
                ...
                <textarea id="observaciones" placeholder="DescripciÃ³n del evento, condiciones del lugar, acciones tomadas..."></textarea>
            </div>
...
<div class="confirm-overlay" id="confirmOverlay">
    <div class="confirm-box">
        ...
        <div class="confirm-title">Â¿Confirmar Emergencia?</div>
        <div class="confirm-text">
            Esta acciÃ³n quedarÃ¡ registrada con fecha, hora y tu usuario.<br>
...
function estadoLabel(e) {
    const m = {
        'ILESO':'âœ… Ileso','HERIDO_LEVE':'ðŸŸ¡ Herido leve','HERIDO_GRAVE':'ðŸŸ  Herido grave',
        'FALLECIDO':'âš« Fallecido','NO_LOCALIZADO':'â“ No localizado','TRASLADADO':'ðŸ”µ Trasladado'
    };
    return m[e] || e;
}
...
// â”€â”€ ARM / SOS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function armSOS() {
    ...
    if (!armed) {
        ...
        document.getElementById('armHint').textContent = 'âš ï¸ Presiona de nuevo para confirmar â€” se cancela en 5s';
        armTimer = setTimeout(() => {
            armed = false;
            btn.classList.remove('armed');
            document.getElementById('armHint').textContent = 'Presiona para armar Â· Segunda pulsaciÃ³n confirma';
        }, 5000);
    } else {
...
// â”€â”€ BUS SEARCH â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let searchTimer;
            ...
            .then(data => {
                ...
                dd.innerHTML = data.map(b => `
                    <div class="bus-option" onclick="selectBus(${JSON.stringify(b).replace(/"/g,'&quot;')})">
                        <div class="bus-name">${b.nombre_bus || 'â€”'} <span class="bus-placa">${b.placa_rodaje || ''}</span></div>
                        <div class="bus-meta">${b.origen || ''} â†’ ${b.destino || ''} &nbsp;|&nbsp; ${b.conductor_1 || ''}</div>
                    </div>`).join('');
...
// â”€â”€ CARGAR PASAJEROS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ RENDER PASAJEROS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function renderPaxList(pax, containerId, modoActivo) {
    ...
    container.innerHTML = pax.map((p, idx) => {
        ...
        const sangre = p.grupo_sanguineo ? `<span class="blood">${p.grupo_sanguineo}</span>` : 'â€”';
        ...
        <div class="pax-medical">
            ...
            <div class="med-item"><div class="med-label">Contacto</div><div class="med-val">${p.contacto_emergencia || 'â€”'}</div></div>
        ...
        <div class="pax-card" data-estado="${estado}" data-paxid="${paxId}">
            <div class="pax-top">
                <div class="pax-info">
                    ...
                    <div class="pax-meta">${p.empresa||''} ${p.cargo ? 'Â· '+p.cargo : ''}</div>
                    <div class="pax-dni">DNI ${p.dni || 'â€”'}</div>
                </div>
...
// â”€â”€ CONFIRMAR Y ACTIVAR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ CARGAR EMERGENCIA ACTIVA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ GUARDAR ESTADO PASAJERO (modo activo) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ CERRAR EMERGENCIA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                <div class="hist-card" onclick="openHistModal(${e.id})">
                    <div class="hist-top">
                        <div>
                            ...
                            <div class="hist-bus">${e.bus_nombre||'â€”'} &nbsp;|&nbsp; ${e.bus_placa||'â€”'}</div>
                        </div>
    ...
    fetch(`?ajax=cargar_emergencia&id=${id}`)
        ...
        .then(res => {
            ...
            document.getElementById('histModalSub').textContent =
                `${e.bus_nombre||''} Â· ${e.bus_placa||''} Â· ${formatDateTime(e.fecha_hora)} Â· ${e.usuario_activa||''}`;
